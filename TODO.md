...
- [ ] **Oversized cards are still reshaped to 63:88, so `sheet` crops 1.01mm off each side.** `bleed.fit`/`fit_plan` and `doctor`'s aspect check all use `cfg.card_w_mm/card_h_mm` unconditionally and never consult `card.oversized`. Measured: a 63.5:88.9 master into an 88.9×127 cell with `cover` is 1.0204× too wide → 1.0% (0.91mm) cropped per side, and the border comes out **2.07/2.12mm where `mtg-oversized` intends 2.98/2.99**. Today's spec work made the *spec* right; nothing consumes it correctly yet. Both call sites want `sheet.trim_mm(card, cfg)` rather than the config pair
