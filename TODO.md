...
- [ ] **Oversized cards are still reshaped to 63:88, so `sheet` crops 1.01mm off each side.** `bleed.fit`/`fit_plan` and `doctor`'s aspect check all use `cfg.card_w_mm/card_h_mm` unconditionally and never consult `card.oversized`. Measured: a 63:88 master into an 88.9×127 cell with `cover` is 1.0227× too wide → 1.14% (1.01mm) cropped per side, and the border comes out **2.07/2.12mm where `mtg-oversized` intends 2.98/2.99**. Today's spec work made the *spec* right; nothing consumes it correctly yet. Both call sites want `sheet.trim_mm(card, cfg)` rather than the config pair
- [ ] **`Config.card_w_mm` says 63×88 and `frames.CARD_MM` says 63.5×88.9 — both claim to be what the games print at, and they cannot both be right.** The trim is what every master is reshaped to and what `sheet` imposes at, so proxdex's proxies come out ~0.8% smaller than a real card (0.5mm narrower, 0.9mm shorter), which shows up sleeved beside real cards. Changing the trim default would invalidate every stored bordered master, so it is a decision rather than a fix — but the `card_w_mm` help text ("Both games print at 63×88 mm") is wrong either way now that the real size has been measured
